        ...
        case p := <-ps.Packets():
            if len(p.Layers()) == 4 {
                ...
                if c[2] == 0x84 && c[3] == 0x00 && c[6] == 0x00 && c[7] == 0x01{
                    // Take IP from network layer (ipv4), regardless of IPv6
...
// According to ip generated mdns request package, the package is stored in the buffer
func mdns(buffer *Buffer, ip string) {
    b := buffer.PrependBytes(12)
    binary.BigEndian.PutUint16(b, uint16(0)) // 0x0000 According to the i logo
...
func sendMdns(ip IP, mhaddr net.HardwareAddr) {
    ...
    if err != nil {
        log.Fatal("Problems with SerializeLayers:", err)
    }
...
// Obtain the host name from the mdns response message
